...
void Tracking4D::initialize() {
    ...
    for(auto& detector : get_regular_detectors(true)) {
        ...
        residualsX_vs_positionX_global[detectorID] =
            new TH2F("GlobalResidualsX_vs_GlobalPositionX",
                     title.c_str(),
                     500,
                     -3 * detector->getPitch().X(),
                     3 * detector->getPitch().X(),
                     400,
                     detector->displacement().X() - detector->getGlobalProjection().X() / 1.5,
                     detector->displacement().X() + detector->getGlobalProjection().X() / 1.5);
        title = detectorID + " global  Residual X vs. global position Y;x-x_{track} [mm];y [mm]";
        residualsX_vs_positionY_global[detectorID] =
            new TH2F("GlobalResidualsX_vs_GlobalPositionY",
                     title.c_str(),
                     500,
                     -3 * detector->getPitch().X(),
                     3 * detector->getPitch().X(),
                     400,
                     detector->displacement().Y() - detector->getGlobalProjection().Y() / 1.5,
                     detector->displacement().X() + detector->getGlobalProjection().Y() / 1.5);
        ...
        residualsY_vs_positionY_global[detectorID] = new TH2F("GlobalResidualsY_vs_GlobalPositionY",
                                                              ...
                                                              3 * detector->getPitch().Y(),
                                                              400,
                                                              -detector->getGlobalProjection().Y() / 1.5,
                                                              detector->getGlobalProjection().Y() / 1.5);
        ...
        residualsY_vs_positionX_global[detectorID] = new TH2F("GlobalResidualsY_vs_GlobalPositionX",
                                                              ...
                                                              3 * detector->getPitch().Y(),
                                                              400,
                                                              -detector->getGlobalProjection().X() / 1.5,
                                                              detector->getGlobalProjection().X() / 1.5);
