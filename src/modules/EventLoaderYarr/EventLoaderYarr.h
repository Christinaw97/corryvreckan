...
namespace corryvreckan {
     ...
     *
     * This module reads a .raw file from a specified input directory (and detector
     * name) and extracts trigger and hit data to be added to the clipboard. It also
     * creates ROOT plots.
     */
    ...
    public:
        /**
         * @brief Constructor: retrieves configuration values (input directory,
        ...
        /**
         * @brief [Initialise this module] : Look for the data file, open it and
         * initialise histograms
         */
        void initialize() override;

        /**
         * @brief [Run the function of this module] : Process a group of trigger
         * events and update the clipboard accordingly
         */
    ...
    private:
        ...
        // Read the header of the event which contains timing and trigger window
        // information
        TriggerHeader read_header();

        // Read the hits (Col, Row, ToT) in an event and creates Pixel objects
        void read_hits(PixelVector& pixels, double eventTime, uint16_t nHits);

        // Find the raw data files in the input directory
        std::string find_raw_file(const std::string& directory, const std::string& detectorName);

        // Adjust the timestamp to account for day rollover
        double adjust_time(const double& current_time);
    };
